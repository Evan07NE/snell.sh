# 此脚本仅用于(自用)远程连接家中局域网内机器，请勿用于任何非法行为！
## 适用于64位Linux系统。
...
apt install -y socat   #Debian/Ubuntu 此条可不装
apt-get install wget   #Debian/Ubuntu 
yum update -y          #CentOS 
yum install -y curl    #CentOS 
yum install -y socat   #CentOS 此条可不装
...
```
docker run -d --env PORT=7770 --env PSK=dFDL0H4NFMOieRyeb6Ly59EJUwrCiEg --env OBFS=tls -p 7770:7770 -p 7770:7770/udp --name snell-server -v /etc/snell/:/etc/snell/ --restart=always primovist/snell-docker
```
其中：
